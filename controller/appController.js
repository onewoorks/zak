...
zakApp.controller("user_navigation", [
    "$scope",
    function($scope) {
        ...
        $scope.logout = () => {
            ...
            window.location.href = './login.html?d=logout'
        }
...
zakApp.service("generalController", function($http, $q) {
    ...
    generalFunction.listOfCawangan = () => {
        return $http.get(api_url + "/cawangan").then(
            function(response) {
    ...
    generalFunction.listOfCawanganLama = () => {
        return $http.get(api_url + "/cawangan/cawangan-zak1").then(
            function(response) {
                if (typeof response.data.result === "object") {
                    return response.data.result
                } else {
    ...
    generalFunction.listOfCawanganHead = () => {
        return $http.get(api_url + "/cawangan/cawangan-head").then(
            function(response) {
                if (typeof response.data.result === "object") {
                    return response.data.result
                } else {
    ...
    generalFunction.listKakitangan = () => {
        return $http.get(api_url + "/cawangan/kakitangan").then(
            function(response) {
                if (typeof response.data.result === "object") {
                    return response.data.result
                } else {
    ...
    generalFunction.listBank = () => {
        return $http.get(api_url + "/bank/bank-list").then(
            function(response) {
                if (typeof response.data.result === "object") {
                    return response.data.result
                } else {
...
zakApp.controller("stokTarikController", [
    "$scope",
    "$http",
    function($scope, $http) {}
])

zakApp.controller("dashboardController", [
    "$scope",
    "$http",
    function($scope, $http) {
...
zakApp.controller("semakanIkutPilihanController", [
    "$scope",
    "$http",
    "generalController",
    function($scope, $http, generalController) {
...
zakApp.controller("alirantunaiController", [
    "$scope",
    "$http",
    "generalController",
    "SweetAlert",
    function($scope, $http, generalController, SweetAlert) {
        angular.extend($scope, generalController)
        $scope.listKakitangan().then(function(response) {
        ...
        var kiraJumlah = () => {
            ...
            angular.forEach(awlist, function(value, key) {
                ...
                berat_emas =
                    parseFloat(berat_emas) + parseFloat(value.emas_berat)
        ...
        addToList = () => {
            var data = {
                ...
                jenis_masuk:
                    $scope.transaksi.jenis === "masuk"
                        ? Number($scope.jumlah || 0)
                        : 0,
                jenis_keluar:
                    $scope.transaksi.jenis === "keluar"
                        ? Number($scope.jumlah || 0)
                        : 0,
        ...
        daftarAw = () => {
            $http({
                ...
                SweetAlert.swal(
                    "Berjaya!",
                    "Pendaftaran aliran wang telah berjaya.",
                    "success"
                )
            })
        }
...
zakApp.controller("stokController", [
    ...
    function($scope, $http, $location, SweetAlert) {
        ...
        var getListCawanganStok = () => {
            $http.get(api_url + "/stok/stok-info").then(function(response) {
        ...
        $scope.viewRecords = caw_id => {
            $http
                .get(api_url + `/stok/stok-info?caw_id=${caw_id}`)
                .then(function(response) {
...
zakApp.controller("rekodJualanController", [
    "$scope",
    "$http",
    "$location",
    "SweetAlert",
    function($scope, $http, $location, SweetAlert) {
        var getListJualan = () => {
            $http.get(api_url + "/transaksi/jualan").then(function(response) {
        ...
        $scope.cetakInvois = resit_no => {
            window.open(
                app_url + "/pages/cetak/resit-jualan.html?id=" + resit_no
            )
        }

        $scope.editInvois = resit_no => {
            window.open(
                app_url + "/home.html#!/ubah-rekod-jualan?id=" + resit_no,
                "_self"
            )
        }
            ...
            SweetAlert.swal(
                {
                    ...
                    text:
                        "Adakah anda pasti untuk membuang invois ini daripada rekod?!",
                    type: "warning",
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        ...
                        SweetAlert.swal(
                            "Pembatalan",
                            "Rekod invois ini tidak dibuang dari sistem",
                            "error"
                        )
                    }
                }
...
zakApp.controller("laporanBulananController", [
    "$scope",
    "$http",
    function($scope, $http) {
        var getLaporanBulan = () => {
            $http
                .get(api_url + "/rekod/transaksi-bulan-ini")
                .then(function(response) {
...
zakApp.controller("laporanHarianController", [
    "$scope",
    "$http",
    function($scope, $http) {
        var getLaporanHarian = () => {
            $http
                .get(api_url + "/rekod/transaksi-hari-ini")
                .then(function(response) {
                    $scope.listAliran = response.data.result
                })
        }
...
zakApp.controller("semakanRekodBakiTerakhir", [
    "$scope",
    "$http",
    "generalController",
    function($scope, $http, generalController) {
...
zakApp.controller("buangRekodTransaksiController", [
    "$scope",
    "$http",
    "SweetAlert",
            ...
            SweetAlert.swal(
                {
                    ...
                    text:
                        "Adakah anda pasti untuk membuang transaksi-transaksi ini daripada rekod?!",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#3e8f3e",
                    confirmButtonText: "Ya Buang!",
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        ...
                        SweetAlert.swal(
                            "Pembatalan",
                            "Rekod transaksi ini tidak dibuang dari sistem",
                            "error"
                        )
                    }
                }
